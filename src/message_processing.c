 ...
 *
 * Single-thread send/receive of 4-unsigned-long queue messages.
 */
#include "tm_api.h"
...
void tm_message_processing_thread_report(void)
{
    ...
    TM_REPORT_LOOP
    {
        ...
        printf(
            "**** Thread-Metric Message Processing Test **** Relative Time: "
            "%lu\n",
            relative_time);
        ...
        if (tm_message_processing_counter == last_counter) {
            printf(
                "ERROR: Invalid counter value(s). Error sending/receiving "
                "messages!\n");
        }
